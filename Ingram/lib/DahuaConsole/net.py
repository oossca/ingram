...
try:
    ...
    from pwn.exceptions import PwnlibException
except ImportError:
    # 当pwn不可用时，remote和PwnlibException将在需要时处理
    remote = None
        ...
        if self.relay_host:
            ...
            try:
                if remote:
                    ...
                    # 当pwn不可用时，创建一个基本的socket连接
                    import socket
                    self.remote = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    self.remote.settimeout(self.timeout)
                    self.remote.connect((self.rhost, self.rport))
            except PwnlibException:
                return False
            except Exception as e:
                print(f"[dh_connect] Error creating connection: {repr(e)}")
                return False
